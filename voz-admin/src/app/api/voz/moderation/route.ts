...
export async function PATCH(request: Request) {
    try {
        const body = await request.json();
        const { employeeName, inactivityAlert } = body;
        ...
        return corsHeaders(NextResponse.json({ 
            error: 'Acceso no autorizado. Los cambios de moderación deben realizarse a través del endpoint de comandos /api/voz/admin/reject-video.' 
        }, { status: 403 }));
    } catch (error) {
        return corsHeaders(NextResponse.json({ error: 'Failed to process request' }, { status: 500 }));
    }
}
