...
export async function PATCH(request: Request) {
    try {
        // SECURITY FIX (VULN-04): Always require Director auth for employee updates.
        // The previous isSelfUpdate bypass was a privilege escalation risk.
        // lastLogin is now updated directly in the login route.
        ...
        // Prevent role escalation: employees cannot set a role higher than their own
        if (updates.role !== undefined && Number(updates.role) > Number(auth.employee?.role)) {
