...
export async function GET() {
    try {
        ...
        // A user's expected balance = (Purchases) + (Gifts/Bonuses received) - (Gifts sent) - (Redemptions to Euro)
        // Note: For now, redemptions is not implemented,        // 4. Calculate individual discrepancies (Per-User Audit)
        ...
        for (const user of users) {
            ...
            // Gifts and donations grant their respective creator splits (65% and 75%)
            ...
            // Since redemptions deduct from wallet immediately when 'pending', we deduct all non-rejected ones.
            ...
            // Allow a tiny float precision error just in case (e.g., fractional coins from 75%)
            // If the user has more coins in their wallet than the math dictates, they injected them.
