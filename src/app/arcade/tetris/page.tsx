...
export default function TetrisArcadePage() {
    const { user } = useAuth();
    const [portalElement, setPortalElement] = useState<HTMLDivElement | null>(null);

    return (
        <div className="container" style={{ padding: '40px 0' }}>
            <h1 style={{ textAlign: 'center', marginBottom: '30px', fontSize: '2.5rem', color: '#9c27b0', textShadow: '2px 2px 0px #fff' }}>
                TETRIS CLASSIC
            </h1>
            ...
            <div style={{ width: '100%', margin: '0 auto' }}>
                ...
                <div style={{ position: 'relative', maxWidth: '2000px', margin: '0 auto' }}>
                    ...
                    <div style={{
                        ...
                        <TetrisGame portalTarget={portalElement} />
                    </div>
                    {/* Portal Target for Game UI */}
                    <div
                        ref={setPortalElement}
                        id="tetris-portal-target"
                ...
                <div style={{ marginTop: '80px', padding: '20px', background: '#fff', borderRadius: '8px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
                    <div className="relative z-10 mt-10 p-6 bg-gray-900/90 rounded-xl border border-purple-500/50 max-w-2xl mx-auto shadow-2xl">
                        <h3 className="text-2xl font-bold text-purple-500 mb-4 tracking-wider">REGLAS DEL JUEGO</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-left text-sm text-gray-300 font-mono">
                            <div className="flex items-center gap-2"><span className="text-xl">🕹️</span> Joystick: Mover y Bajar Rápido.</div>
                            <div className="flex items-center gap-2"><span className="text-xl">↻</span> Botón A: Rotar pieza.</div>
                            <div className="flex items-center gap-2"><span className="text-xl">🔻</span> Botón B: Caída fuerte (Hard Drop).</div>
                            <div className="flex items-center gap-2"><span className="text-xl">💰</span> Sobrevive para ganar ROBcoins.</div>
                        </div>
                    </div>
                </div>
            </div>
